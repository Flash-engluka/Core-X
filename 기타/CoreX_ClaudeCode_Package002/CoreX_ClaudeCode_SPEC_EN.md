...
**Usage context:** This project is intended for **private, personal, non-commercial use** by the user only. It uses the actual Undertale character images and OST as-is (external distribution, public hosting, and monetization are prohibited).
...
### 7-5. Item Board Placement (finalized)

**Visibility rules (Fog of War):**
- **Special items (faction characters)**: always visible to everyone
- **Regular items**: shown as `???` by default; identity is revealed only when a player arrives on that cell. **When the player leaves the cell, it reverts to `???`** (= you only "know" while standing there)

**Respawn**: at round end, items respawn at the **same fixed positions** every time

**Regular item placement (by simple number / zone)**:
- Froggit (×3): zone 1 (precise 19) ×2, zone 2 (precise 10) ×1
- Whimsun (×3): zone 1 (19) ×1, zone 2 (10) ×1, zone 7 (16) ×1
- Lesser Dog (×4): zone 1 (19) ×2, zone 5 (30) ×1, zone 9 (23) ×1
- Moldsmal (×3): zone 4 (21) ×1, zone 6 (31) ×1, zone 8 (17) ×1

→ The asymmetric placement is **intentional design** (exploration element + offsets the top factions' Core proximity advantage by making travel tedious). Do NOT modify arbitrarily.

**Special item placement (diagonal pairing)**:
- Napstablook → WATERFALL faction cell (Toriel only, -10G)
- Sans → HOTLAND faction cell (Papyrus only, -5G)
- Flowey → HOTLAND faction cell (Papyrus only, -7G)
- Asgore → RUINS faction cell (Undyne only, -10G)
- Monster Kid → SNOWDIN faction cell (Mettaton only, -10G)

> Each character must travel to the **diagonally opposite faction** (not their own) to pick up their special item. (RUINS↔WATERFALL, SNOWDIN↔HOTLAND)

### 7-6. Pickup Rights

| Character | Items they can pick up |
|-----------|-------------------------|
| Toriel | Froggit, Whimsun (own To-do), Napstablook |
| Papyrus | Lesser Dog (own To-do), Sans, Flowey |
| Undyne | Moldsmal (own To-do), Asgore |
| Mettaton | Monster Kid (Mettaton has no regular-item To-do — focuses on Core upgrades) |

- Regular items: only the listed character can pick them up (others can travel to the cell but cannot pick them up). Free of cost.
- Special items: only the listed character, with the specified G cost.
- Picking up consumes the turn's action (move OR pick — only one).
...
### Nickname input (finalized)
- Online room create/join screen nickname input field: **placeholder must be exactly `FRISK`** (uppercase, 5 letters)
- maxlength = 10
- If user proceeds without entering, default to `FRISK`

### Language toggle (finalized)
- **Korean ↔ English toggle** available (KO/EN toggle button at the top-right of every screen)
- Default language: auto-detect from browser (fallback to Korean)
- **Proper nouns are always English regardless of language**: place names (Ruins, Snowdin, Waterfall, Hotland, Core), items (Froggit, Whimsalot, Napstablook, etc.), characters / player names (Toriel, Papyrus, Undyne, Mettaton, Frisk, Asriel, etc.)
- What gets translated: button labels (Move/이동, Pick/줍기, End Turn/턴 종료, etc.), toast/modal messages, help text
- Implementation: a small custom `i18n` module or a simple JS object (`{ko:{...}, en:{...}}`) is enough

### Cell co-occupancy (finalized)
- **Multiple players may occupy the same cell** (no restriction)
- Visual treatment: when multiple tokens share a cell, offset them slightly so all are visible (2 = side-by-side, 3 = triangle, 4 = square arrangement)

### Upgrade modal (finalized)
- The modal **opens automatically** the moment a player arrives at the Core cell (no separate [Upgrade] button click needed)
- Modal content: available upgrade options + [Upgrade] confirm button + [Close]
- **Option display rules**:
  - Character upgrade option: only shown if the **current character has a next tier**
    - Undyne → Pre-Undying / Pre-Undying → Undyne the Undying (Undyne the Undying has no option)
    - Mettaton → Mettaton EX / Mettaton EX → Mettaton NEO
    - **The "Call Alphys" option is unlocked only after reaching Mettaton NEO** (Mettaton NEO → Mettaton NEO + Alphys). The modal label should read **"Call Alphys"** rather than "Upgrade to Mettaton NEO + Alphys"
    - Mettaton NEO + Alphys: no option (final)
    - Toriel · Papyrus: no character upgrade option at all
  - Item upgrade option: only items currently in the inventory that **are upgradeable** are shown
    - Froggit, Whimsun, Lesser Dog, Moldsmal (→ Final Froggit, Whimsalot, Greater Dog, Moldbygg)
    - Already-final items (Final Froggit, etc.) and special items have no option
- **Selection limit**: choose **exactly one** total across character and items (1 upgrade token = 1 target)
- **Close / no-use = upgrade token disappears immediately** (per original PDF rule "cannot be stockpiled")

### Result screen rematch (finalized)
- In addition to "Back to main", add a **"Play again with same members" button**
- Behavior: fully reset all G / inventory / Box / position / turn — same 4 players in same factions start a new game
- Online mode: when the host clicks rematch, show a "Rematch invitation" modal to the other 3; the game starts when all accept
- **To start with new members**, go back to main → re-enter via room code
...
### Art style
Pixel art applied throughout. Force `image-rendering: pixelated`. English uses Papyrus / Comic Sans, Korean uses system fonts (see Fonts section).
...
### Fonts (finalized) — Undertale-themed

| Use | Font | Note |
|-----|------|------|
| **English titles / large headings** | `Papyrus`, fantasy | "CORE X #1" main title. Reflects the Papyrus character's name-font joke |
| **English body / buttons / labels / proper nouns** | `'Comic Sans MS', 'Comic Sans', cursive` | Includes Ruins, Froggit, Frisk, etc. Reflects the Sans character's name-font joke |
| **Korean body / notifications** | `'굴림', 'Gulim', system-ui, sans-serif` | |
| **Final leaderboard (Korean)** | `'돋움', 'Dotum', system-ui, sans-serif` | |

**Implementation notes:**
- Papyrus and Comic Sans MS are **system fonts** installed by default on macOS/Windows — no Google Fonts loading required. Both have safe fallbacks.
- Define as CSS variables:
  ```css
  --f-title: 'Papyrus', fantasy;
  --f-body-en: 'Comic Sans MS', 'Comic Sans', cursive;
  --f-body-ko: '굴림', 'Gulim', system-ui, sans-serif;
  --f-leaderboard-ko: '돋움', 'Dotum', system-ui, sans-serif;
  ```
- Toggling the language auto-switches the body font between KO and EN.
- The pixel art + Papyrus/Comic Sans combination matches the **original Undertale visual tone** (the name-font jokes — Sans uses Comic Sans, Papyrus uses Papyrus font).
...
### Actual Sprites (finalized)

All sprites are provided by the user. Folder structure under `assets/sprites/`:

```
assets/sprites/
├── characters/    # 9 files — player characters + upgrade paths
│   ├── Toriel.png
│   ├── Papyrus.png
│   ├── Undyne.png · Pre Undying.png · Undyne the Undying.png
│   └── Mettaton.png · Mettaton EX.png · Mettaton NEO.png · Alphys.png
├── items/         # 13 files — regular items (before/after upgrade) + special items
│   ├── Froggit.png · Final Froggit.png
│   ├── Whimsun.png · Whimsalot.png
│   ├── Lesser Dog.png · Greater Dog.png
│   ├── Moldsmal.png · Moldbygg.png
│   └── Napstablook.png · Sans.png · Flowey.png · Asgore.png · Kid.png
└── asriel/        # 2 files — title screen + victory screen
    ├── Asriel_A.webp   # Title screen decoration
    └── Asriel_B.gif    # Final victory effect (51 frames, ~3s loop)
```

**Image characteristics:**
- 22 character/item files: 1587×2245px, **monochrome (black background + white lines)**, RGB PNG
- Asriel_A: 360×651px, RGBA webp, single frame (young Asriel — the one to be saved)
- Asriel_B: 640×247px, **color (red/green wings)** GIF, 51 frames × 60ms (Hyperdeath Asriel — intentionally kept in color)

**Where each is used:**
| Screen / element | Image used | Recommended size |
|------------------|-----------|------------------|
| Title (main) screen | Asriel_A.webp | Large, on one side + gentle float animation |
| Final victory effect | Asriel_B.gif | Centered, large, GIF plays natively + scale-in 1s |
| Character select screen | characters/*.png | ~240px preview |
| HUD portrait (right panel) | current character stage PNG | ~120px |
| Board tokens (player markers) | characters/*.png | ~64px (paired with a faction-color ring) |
| Inventory / Box slots | items/*.png | ~48px |
| Upgrade modal | before/after PNG | Large + fade/scale transition |
| Regular item cells (fog) | `???` text only | (no image) |

**Rendering treatment:**
- Apply `image-rendering: pixelated` as before (preserve pixel art)
- **Recommended: tint the monochrome characters/items with faction colors:**
  - Toriel · Asgore · RUINS tokens → red `#ff0000` tint
  - Papyrus · Sans · Flowey · SNOWDIN tokens → blue `#0099ff` tint
  - Undyne (early stages) · Napstablook · WATERFALL tokens → green `#15ff00` tint
  - Mettaton (early stages) · Kid · HOTLAND tokens → yellow `#fbff00` tint
  - **Implementation:** when drawing on Canvas, use `globalCompositeOperation = 'source-atop'` + color fill, or use CSS `filter`. Note: Asriel_B keeps its native colors (no tint).
- Keep the existing 3-second float idle animation on characters.

### Character → Sprite mapping

The game code must select the right image based on the player's `character` field:

```
toriel              → characters/Toriel.png
papyrus             → characters/Papyrus.png
undyne              → characters/Undyne.png
pre-undying         → characters/Pre Undying.png
undyne-the-undying  → characters/Undyne the Undying.png
mettaton            → characters/Mettaton.png
mettaton-ex         → characters/Mettaton EX.png
mettaton-neo        → characters/Mettaton NEO.png
mettaton-neo-alphys → display BOTH Mettaton NEO and Alphys side by side
```

> The final `mettaton-neo-alphys` stage is "NEO + Alphys" per spec, so **display the two images side by side or overlapped** (there is no single combined image).

---

## 12. Sound (finalized)

| Type | Track | OST # | Source |
|------|-------|-------|--------|
| BGM (in-game) | **Hopes and Dreams** | UNDERTALE OST #87 | https://youtu.be/wbO3p7_Mf30 |
| Win SFX (final victory) | **SAVE the World** | UNDERTALE OST #89 | https://youtu.be/LAn-JYzKm5M |
| Lose SFX (final defeat) | **Determination** | UNDERTALE OST #11 | https://youtu.be/6S21ZSsC21U |

**Implementation (Howler.js recommended):**
- Download the three tracks as mp3 or ogg → place under `assets/audio/`
  - Recommended filenames: `bgm_hopes_and_dreams.mp3`, `sfx_save_the_world.mp3`, `sfx_determination.mp3`
- BGM: loop=true at in-game start; fade out on entering the result screen
- Win SFX: when **you yourself** reach 100G, stop BGM and play the win SFX immediately
- Lose SFX: when **another player reaches 100G first and your loss is locked in**, stop BGM and play Determination
- Volume slider in a settings modal — optional

**Since this is a private personal project, use the original tracks as-is** — external distribution, public hosting, and monetization are prohibited (see §16).
...
### Disconnect / dropout handling (finalized)
- During play, if a player disconnects or fails to respond for ≥5 seconds:
  1. Immediately show a **toast notification** to all remaining players: "{nickname} disconnected — AI will take over"
  2. **An AI (auto-bot) takes over that seat** — every turn it auto-passes (no move, no pick, no upgrade, no joker use) → immediately ends the turn
  3. If the disconnected player **rejoins with the same room code, their seat is automatically restored** (AI release; from the next turn the human controls again)
- If the host disconnects: host privilege transfers automatically to the next player by turn order
...
└── assets/
    ├── sprites/
    │   ├── characters/  # 9 files (Toriel, Papyrus, Undyne 3 stages, Mettaton 4 stages)
    │   ├── items/       # 13 files (8 regular + 5 special)
    │   └── asriel/      # Asriel_A.webp (title) + Asriel_B.gif (victory)
...
- **Mobile in-game screen (finalized)**: for board readability, **recommend landscape orientation**. Show a one-time toast "Please rotate your device to landscape" when in portrait mode.
- **Board drag-pan**: if the board is clipped on small screens, **mouse-drag / touch-drag pans the board viewport**. Pinch-zoom optional (not required).
...
- **Load actual sprites** (all 24 files under `assets/sprites/`) — including faction-color tinting
- Place Asriel_A on the title screen
- **Verify**: all screen transitions + starfield + 9 characters appear on the character select screen
...
### M4 — Items & actions
- Render items based on each node's `items` array
  - Regular items: shown as `???`; revealed on arrival; reverts to `???` when leaving
  - Special items: always shown as icons on the faction cells
- Pick-up action: enforce `characterPickupRights`; deduct G for specials
- Upgrade action (use the single upgrade token immediately on leaving the Core — character OR one item)
- Swap action (faction Box ↔ inventory, starting the turn after arrival)
- Auto To-do progression (detect completion on inventory/character changes)
- Joker ② steal (designate target + notify)
...
### M6 — Polish (Release)
- Sound (Web Audio 8-bit generated tones or royalty-free BGM/SFX)
- Final victory: Asriel_B GIF effect (scale-in 1s + play GIF natively)
- Polish upgrade modal before/after fade/scale transitions
...
- Violent depictions
- **External distribution / public hosting / monetization** — this game is for private personal use and bundles official Undertale assets (images, OST) as-is, so public hosting (GitHub Pages, itch.io, Vercel, etc.) or monetization of any kind is prohibited
...
| Item | Status |
|------|--------|
| ~~Regular item board placement (Q3)~~ | ✅ Confirmed (§7-5) |
| ~~Character images~~ | ✅ Delivered (`assets/sprites/characters/`, 9 files) |
| ~~Asriel images~~ | ✅ Delivered (`assets/sprites/asriel/`, A=title / B=victory) |
| ~~Item images~~ | ✅ Delivered (`assets/sprites/items/`, 13 files) |
| ~~Sound files~~ | ✅ Confirmed — BGM: Hopes and Dreams (#87) / Win: SAVE the World (#89) / Lose: Determination (#11) |
| Firebase config | User to enter directly (local mode works fine in placeholder state) |

**Remaining unconfirmed item**: only Firebase config (user enters directly; only needed for online mode). Every other game specification and asset is finalized, so Claude Code can proceed through M1–M6 without blockers.
